...
const RecentTickets = ({ ticketsList, setActiveTab }) => {
    return (
        <div className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden flex flex-col mt-8">
            <div className="border-b border-zinc-100 px-6 py-5 flex items-center justify-between">
                <div>
                    <h2 className="text-base font-semibold text-zinc-950">Recent Tickets</h2>
                    <p className="text-sm text-zinc-500 mt-0.5">Latest 10 support requests</p>
                </div>
                <button 
                    onClick={() => setActiveTab('tickets')}
                    className="text-sm font-medium text-black border border-zinc-200 rounded-md px-4 py-1.5 hover:bg-zinc-100 transition-colors"
                >
                    View All
                </button>
            </div>
