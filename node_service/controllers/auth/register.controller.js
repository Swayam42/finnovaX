...
exports.register = async (req, res) => {
    try {
        ...
        // Send Welcome Email asynchronously in background (fire-and-forget so registration responds instantly in <100ms)
        sesService.sendEmail({
            ...
            `
        }).catch(err => console.error('Failed to send welcome email in background:', err.message || err));
        ...
        if (newUser.phoneNumber) {
            ...
            }).catch(err => console.error('Failed to send welcome SMS in background:', err.message || err));
        }
